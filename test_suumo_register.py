...
def test_parse_score():
    ...
    # ★実機の**文字の並び**を固定するのが目的なので、物件コードと物件名はダミーにする。
    #   このリポは Public。実コードは掲載中の広告を特定できるので置かない
    #   （他のテストも 1005210000xx のダミーで揃えている）。桁数と前後の語順は実機どおり。
    real = ("新規物件登録完了 棟情報 |お金・駐車場等 |契約条件 |間取り 名寄せスコア 28 点 "
            "登録完了しました。 物件コード:100521000001 1 棟情報 物件名 サンプルレジデンス")
    ...
    _check("物件コードも同様に取れる",
           R.extract_bukken_code("物件コード:100521000001") == "100521000001")
