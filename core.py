...
def build_staging_prompt(style_desc: str, room_use: str = "",
    ...
    elif concept_staging:
        # ★staging-furni-v1：特集の方向づけがあるとき、用途推定文は出さない。
        #   この文は concept_staging より『前』に置かれ、特集は後ろに足されるだけで上書きしない。
        #   実測（難波 1LDK 35.36㎡・洋室4.6帖 を②自分を整える部屋で生成）＝洋室にベッドが出た。
        #   ②の staging_prompt にベッドの指示はどこにも無い＝特集が家具を決められない構造だった。
        #   抽象的な優先順の指示を足すのではなく、競合する文をそもそも出さない（確実な方を採る）。
        #   ★用途に合わない家具の禁止（末尾【厳守】のトイレ/浴室のソファ・水回りのベッド）は不変。
        #   ★room_use が明示指定（リビング/寝室）のときは上の分岐が勝つ＝人が選んだものを特集で上書きしない。
        furni = ""
    else:
...
FEATURES = {
    ...
    "hobby": {
        ...
        "staging_prompt": (
            ...
            # ★hobby-relax-v1：趣味アイテムを『必ず1つ置く』とは読ませない（谷合さん判断 2026-08-04）。
            #   実マイソク16件が全て 1K〜1LDK で2室目ゼロ＝25㎡の1DKに趣味を置くと窮屈になるだけ。
            #   ③の要点は「趣味を置ける余白があること」であって、趣味アイテムを置くことではない。
            "- 趣味の選定: 1物件1趣味。部屋の広さ・形状から逆算して1つだけ選ぶ"
            "（デスク／本と棚／自転車／楽器／プロジェクター等）。2つ以上を同じ物件に混ぜない。\n"
            "  ★置くと窮屈になる広さなら、趣味アイテムは置かない。床と壁の余白をそのまま見せる"
            "（③の要点は『趣味を置ける余白があること』であって、趣味アイテムを置くことではない）。\n"
...
_AREA_ROMAJI = {
    ...
    # ── area-dict-v1（2026-08-04）：8月の投稿カレンダーと突合し、辞書に無くて日本語のまま
    #   マストヘッドに焼かれる駅を追加。表記は谷合さん判断で確定・既存29駅は1行も触っていない。
    #   ★『谷町』は登録済みだが _area_romaji は最長『後方』一致（JR福島駅の接頭辞を落とすための設計）
    #     なので、後ろに文字が付く『谷町九丁目』は拾えない。仕様どおりの挙動で、行を足すのが正しい対処。
    #   ★谷町九丁目/四丁目を別表記にするのは、両方 TANIMACHI にすると 8/4難波 と 8/6農人橋 が
    #     同じ表記になり数値分析シートと突合できなくなるため（短さと駅の区別の両立）。
    "谷町九丁目": "TANIMACHI-9", "谷町四丁目": "TANIMACHI-4",
    "日本橋": "NIPPOMBASHI",              # 大阪メトロ公式表記（東京の NIHOMBASHI と区別）
    "玉川": "TAMAGAWA", "玉造": "TAMATSUKURI",
    "天神橋筋六丁目": "TENROKU",           # 公式 TENJIMBASHISUJI-6CHOME は22字でマストヘッドが潰れる
    "野田阪神": "NODA-HANSHIN",            # ★『野田』とは別駅（後方一致で拾えないので独立行）
    "松屋町": "MATSUYAMACHI",
}
...
def data_note_date(facts: dict, gen_date_str: str = "") -> str:
    """★注記の年月：マイソクに『情報の時点』の明示があればそれ、無ければ生成日（呼出側が渡す）。
    景表法『現況優先』併記前提。返り値は『YYYY年M月』。

    ★note-date-v1（8/4 実運用の投稿ブロッカー）: 完成動画の3面すべてに『※2009年12月時点の情報』と
      焼かれていた。2009年12月は築年月であって募集情報の時点ではない（17年前の情報として掲載する状態）。
      v79-note で表紙の年月を『生成日固定』から『マイソク優先』へ変えたとき、緩い年月マッチが
      full_text 内のどの年月でも拾うようになったのが原因。

    決定規則（この順に見る。1つ前で決まったら後ろは見ない）:
      ① 情報の時点として明示された日付（情報日付／更新日／掲載日／募集日／『YYYY年M月時点』等）
      ② 上記が無ければ、full_text 中の年月のうち『築年月ではない』かつ『生成年より古すぎない』もの
      ③ それも無ければ 生成日（gen_date_str）
    ★『無ければ築年月』というフォールバックは作らない。取れないものを別の値で騙らない
      （issue-v1 の「エリアが取れないとき既定エリアを騙らず ISSUE NN 単独へ」と同じ原則）。
    ★築年月の除外は2段構え: facts の built / age に入っている値そのものを候補から外す（キー由来の除外）と、
      本文で築系ラベルの近傍にある年月を外す（文脈由来の除外）。片方だけだとマイソクの書式差で抜ける。"""
    import re as _re
    _ft = str(facts.get("full_text", ""))
    ...
    _jiten = _re.search(r"(\d{4})\s*年\s*(\d{1,2})\s*月\s*(?:\d{1,2}\s*日)?\s*時点", _ft)
    if _jiten:
        return f"{_jiten.group(1)}年{int(_jiten.group(2))}月"

    # ── ② 緩い年月（築年月を2段で除外し、古すぎる年も採らない）
    _built = " ".join(str(facts.get(k, "") or "") for k in ("built", "age", "築年月"))
    _built_ym = set()
...
# ★suumo-export-v2：SUUMO入稿から除外する処理種別。
#   実写真をベースにしていない＝『実際の物件を撮影した写真』ではないため、注記の有無にかかわらず
#   ポータル掲載に使えない（不動産公正取引協議会の表示規約／掲載写真が実際の物件と異なる＝おとり広告）。
#   ・補完生成＝間取り図から部屋を描き起こす／3Dパース＝俯瞰CG。どちらも実在の部屋を撮った写真ではない。
#   ・完成予想図なら明記して掲載できるが、それは未完成物件の話。既存物件の内観として描き起こした画像は
#     その枠に入らない。名寄せスコア（写真14点）のために実在しない部屋のCGで点数を埋めるのは本末転倒。
#   ★家具ステージング（実写真＋AI家具）と高解像度化（実写真の補正）は実写真ベースなので除外しない。
#   ★リノベ後イメージも実写真ベース＝『リフォーム後イメージ』と明記すれば掲載可なので除外しない。
#   ★値は app.py の treatment と一致させること（『3Dパース（試験）』＝（試験）まで含めて1つの値）。
#     ここがずれると黙って除外されなくなるので、定義の直後に自己照合を置いてある（suumo_excluded の doctest 相当）。
SUUMO_EXCLUDE_TREATMENTS = ("補完生成", "3Dパース（試験）")


def suumo_excluded(treatment) -> bool:
    """その処理種別は SUUMO 入稿用ZIPから除外するか。
    ★判定は treatment（処理種別）で行い、disc の文言では判定しない
      ＝注記の文言を直したときに判定が壊れないため。"""
    return str(treatment or "").strip() in SUUMO_EXCLUDE_TREATMENTS
