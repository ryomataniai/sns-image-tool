...
def _pl_stage_review():
    ...
    if adopted:
        ...
        with s1:
            ...
            _n_photo = len(adopted) + (1 if _sfp_on else 0)
            _cs = {core.suumo_category(it.get("room", "")) for it in adopted}
            if _sfp_on:
                _cs.add("madori")
            ...
            if _n_photo >= 14 and _cats >= 5:
                ...
                st.warning(f"写真{_n_photo}点 / {_cats}カテゴリ ― "
                           ...
                           f"（実測: 14点=34点／13点=33点／10点=18〜32点）")
        with s2:
            _sbuf = io.BytesIO()
            with zipfile.ZipFile(_sbuf, "w", zipfile.ZIP_DEFLATED) as _zf:
                _used = set()
                for _k, _it in enumerate(adopted, 1):
                    _sb = _it["gen_bytes"]
                ...
                if _sfp_on:   # 間取り図＝実物（生成AI非通過）なので注記なし。変換とファイル名だけ揃える
                    _zf.writestr(core.suumo_filename(len(adopted) + 1, "間取り図", _used),
