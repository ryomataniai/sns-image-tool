...
def _pl_mag_generate_cb(feature_id, brand_id=core.DEFAULT_BRAND):
    ...
    res = core.magtext(
        ...
        brand_id=brand_id)                        # ★brand-v1：プロンプトの誌名（生成トーンが誌名に引きずられる）
    if not res:
...
def _pl_pregen_summary(adopted):
    ...
    # ★issue-gate-v1：ISSUE番号／エリアは3面に焼かれ job_id にも入る＝文字面と並ぶ最上段の関門。
    #   未設定（0）は赤で出し NG に積む（🎬ボタンも押せない）。エリアは自動取得できるので警告どまり。
    if not _pl_issue_ready():
        _parts.append("**ISSUE番号＝🔴未設定**（📖で入力してください・3面に焼かれます）")
        _ng.append("ISSUE未設定")
    else:
        _iss = _pl_issue_text()
        _parts.append(f"雑誌表記={_iss}"
                      + ("" if "/" in _iss else "　⚠️エリア未取得（手入力で補えます）"))
...
# ★dup-treatment-v1：同じ元画像から2版（高画質化版／家具ステージング版）を作れるようにする。
#   SUUMOの名寄せスコアは写真点数でほぼ決まる（実測 14点=34点＝第1基準）が、マイソクから取れる写真は
#   10点前後しかない。同じ部屋を2版にすれば点数を足せる（実在する部屋の写真なので規約上の問題もない）。
#   ★複製の既定 treatment は「元と別のもの」＝押した瞬間に意味のある組み合わせになる。
_PL_DUP_PAIR = {"家具ステージング": "高解像度化のみ", "高解像度化のみ": "家具ステージング",
                "リノベ後イメージ": "高解像度化のみ", "水回り・玄関を演出": "高解像度化のみ"}


def _pl_duplicate_item(iid):
    """★dup-treatment-v1：同じ元画像を参照する新アイテムを、複製元の『直後』に挿入する（on_click）。
    ★コールバック内なので session_state（ウィジェットキー）への代入が安全＝widget生成前に確定する。
    ★src_bytes は複製元と同じものを共有（再アップロード不要・メモリも増やさない）。
    ★disc は引き継がない。treatment が変わると注記も変わるため、複製時は必ず None から始める
      （引き継ぐと『高解像度化のみ』なのに CG注記が焼かれた画像ができる）。
    ★order は複製元の直後。後続を1つずつ後ろへずらす＝手動並べ替え(_pl_move)とも自動整列とも衝突しない
      （自動整列は同ランク内で現orderを保つ安定ソートなので、同じ部屋の複製ペアは隣り合ったまま）。"""
    items = st.session_state.get("pl_items", [])
    src = next((x for x in items if x["id"] == iid), None)
    if src is None:
        return
    new_id = max((x["id"] for x in items), default=-1) + 1
    _base_order = src.get("order", 0)
    for x in items:                                   # 複製元より後ろを1つずつ空ける
        if x.get("order", 0) > _base_order:
            x["order"] = x.get("order", 0) + 1
    _treat = _PL_DUP_PAIR.get(src.get("treatment", ""), "高解像度化のみ")
    dup = {"id": new_id, "order": _base_order + 1, "src_bytes": src["src_bytes"],
           "room": src.get("room", "その他"), "treatment": _treat,
           "gen_bytes": None, "caption": "", "disc": None,     # ★disc は引き継がない
           "jo": src.get("jo"), "room_id": src.get("room_id"),
           "_codes": list(src.get("_codes") or []), "_raw_codes": list(src.get("_raw_codes") or []),
           "_origin": src.get("_origin", "photo"), "_dup_of": iid}
    items.insert(items.index(src) + 1, dup)
    st.session_state["pl_items"] = items
    # ウィジェットの値を先に確定させる（この代入が無いと selectbox が先頭候補に落ちる）
    st.session_state[f"pl_treat_{new_id}"] = _treat
    if dup["room_id"] is not None:
        st.session_state[f"pl_roomid_{new_id}"] = dup["room_id"]
    else:
        st.session_state[f"pl_room_{new_id}"] = dup["room"]
    st.session_state["_pl_dup_msg"] = (
        f"複製しました（{dup['room']}・{_treat}）。処理を変えて2版にできます。"
        "★生成枚数が1枚増えます（下のボタンの概算コストで確認できます）。")


def _pl_delete_item(iid):
    """★dup-treatment-v1：複製アイテムを削除する（on_click）。複製元（_dup_of なし）は消さない
    ＝取り込んだ写真そのものを消す導線はここに作らない（取り込みのやり直しは『最初からやり直す』が担当）。"""
    items = st.session_state.get("pl_items", [])
    tgt = next((x for x in items if x["id"] == iid and x.get("_dup_of") is not None), None)
    if tgt is None:
        return
    st.session_state["pl_items"] = [x for x in items if x["id"] != iid]
    for k in (f"pl_treat_{iid}", f"pl_room_{iid}", f"pl_roomid_{iid}"):
        st.session_state.pop(k, None)
    st.session_state["_pl_dup_msg"] = "複製を削除しました。"
...
def _pl_stage_input():
    ...
    _dupmsg = st.session_state.pop("_pl_dup_msg", None)
    if _dupmsg:
        st.info(_dupmsg)
    ...
    for it in _photo_items:
        ...
        # ★dup-treatment-v1：同じ元画像から2版を作る。複製されたものだけ削除できる
        #   （取り込み写真そのものを消す導線はここに作らない）。
        if it.get("_dup_of") is None:
            rc4.button("複製", key=f"pl_dup_{i}", on_click=_pl_duplicate_item, args=(i,),
                       help="同じ写真から別の処理版をもう1枚作る（SUUMOの写真点数を増やせます）",
                       use_container_width=True)
        else:
            rc4.button("複製を削除", key=f"pl_dupdel_{i}", on_click=_pl_delete_item, args=(i,),
                       use_container_width=True)
    ...
    # ★dup-treatment-v1：複製が何枚含まれているかを内訳に出す（コストは既に約$表示があるが、
    #   『なぜ枚数が増えたのか』が分からないと増やしすぎに気づけない）。
    _n_dup = len([it for it in _photo_jobs if it.get("_dup_of") is not None])
    _parts = [f"写真{_n_photo}枚" + (f"（うち複製{_n_dup}枚）" if _n_dup else "")]
    if _n_gap:
...
def _pl_stage_review():
    ...
    # ── SUUMO入稿用ZIP（suumo-export-v1）─────────────────────────────
    #   ★上の「画像だけ保存（ZIP）」とは別物。既存ZIPにも動画にも1行も手を入れていない
    #     （分岐フラグでモードを切り替えると既存経路に if が入って回帰の余地ができるため、
    #      ボタンごと別に立てる方式にした）。
    #   SUUMOの受付条件＝JPEGのみ／ファイル名は半角英数／注記は規約の2要素（CG生成・家具は付属しない）。
    if adopted:
        _sfp = st.session_state.get("pl_floorplan")
        _sfp_on = bool(st.session_state.get("pl_include_fp") and _sfp is not None)
        # ★suumo-export-v2：描き起こし画像（補完生成／3Dパース）は SUUMO 入稿から外す。
        #   実在の部屋を撮った写真ではないため、注記を直しても掲載可否は変わらない
        #   （表示規約＝掲載写真が実際の物件と異なる＝おとり広告）。判定は core 側の定数で持ち、
        #   disc の文言には依存させない（文言を直したときに黙って壊れないように）。
        #   ★SNS用の「画像だけ保存（ZIP）」からは外さない＝SNS用途では問題なく使えるため。
        _s_adopt = [it for it in adopted if not core.suumo_excluded(it.get("treatment"))]
        _s_excl = [it for it in adopted if core.suumo_excluded(it.get("treatment"))]
        s1, s2 = st.columns([2, 1])
        with s1:
            # ★名寄せスコアの目安。実測（掲載中27件）で 14点=34点（第1基準）／13点=33／10点=18〜32。
            #   点数だけでなくカテゴリ数も効いていると見られる（正確な式はSUUMO非公開・確信度 中）。
            #   ★間取り図を添付する回は madori カテゴリとして数える（採用画像に間取り図は含まれないため、
            #     ここで足さないとカテゴリ数を過少に見せて、届いているのに警告が出る）。
            #   ★点数・カテゴリは『除外後』で数える。除外前で数えると、実際にSUUMOへ入るのは
            #     12点なのに「14点」と表示され、判定そのものが嘘になる（ここが v2 の実質的な本題）。
            _n_photo = len(_s_adopt) + (1 if _sfp_on else 0)
            _cs = {core.suumo_category(it.get("room", "")) for it in _s_adopt}
            if _sfp_on:
                _cs.add("madori")
            _cats = len(_cs & set(core.SUUMO_CATEGORIES))   # ★other は5カテゴリに数えない
            _excl_note = f"（うち{len(_s_excl)}枚はSUUMO入稿から除外）" if _s_excl else ""
            if _n_photo >= 14 and _cats >= 5:
                st.success(f"写真{_n_photo}点 / {_cats}カテゴリ ― 名寄せ第1基準（34点）に届く見込み"
                           + _excl_note)
            else:
                st.warning(f"写真{_n_photo}点 / {_cats}カテゴリ ― "
                           f"14点・5カテゴリで名寄せ第1基準（34点）に届きます"
                           f"（実測: 14点=34点／13点=33点／10点=18〜32点）" + _excl_note)
            if _s_excl:   # ★黙って減らさない。何を何枚外したかを理由つきで出す
                _by = {}
                for _e in _s_excl:
                    _t = str(_e.get("treatment", "") or "")
                    _by[_t] = _by.get(_t, 0) + 1
                st.info(f"SUUMO入稿用ZIPから{len(_s_excl)}枚を除外しました（"
                        + "・".join(f"{_t}{_n}枚" for _t, _n in _by.items()) + "）\n\n"
...
def _pl_brand_id():
    """★brand-v1：選択中のブランドid。既定は用途モード（pl_mode）に連動し、手動選択があればそれを優先。
    ★自動だけにすると『誤った側のブランドで焼いた』ことに気づけないので、UIで上書きできる形にしている
      （issue-v1 のエリア表記と同じ考え方）。"""
    _manual = str(st.session_state.get("pl_brand", "") or "").strip()
    if _manual in core.BRANDS:
        return _manual
    return core.brand_for_mode(st.session_state.get("pl_mode", ""))


def _pl_brand_masthead():
    """★brand-v1：マストヘッドに実際に焼かれる文字列を返す唯一の入口。
    UIプレビュー／表紙／ビート面／DATA面の4消費がこれを見る＝『見えている文字列＝焼かれる文字列』。"""
    return core.brand_masthead(_pl_brand_id())


def _pl_issue_ready():
    """★issue-gate-v1：ISSUE番号が『人の手で入力されたか』。0/空＝未設定。
    ★Streamlit Cloud はセッションが切れると session_state が全部リセットされ、既定値に戻る。
      ISSUE番号は表紙・全ビート・DATA面の3面に焼かれ、しかも job_id に入るので
      間違えたまま生成すると直すのに fal 再課金（$2.45）。既定 1 が正しいのは創刊号だけなので、
      『既定値のまま押せてしまう』状態自体を無くす（警告では毎回見落とす）。
    ★永続化はしない＝別物件の号数を引き継ぐ事故を作らない。人が毎回明示的に入れる。"""
    try:
        return int(st.session_state.get("pl_issue_no", 0) or 0) >= 1
...
def _pl_v79_area_line(facts):
    ...
    # ★brand-v1：誌名の直書きをやめ、選択中のブランドから引く。
    return _pl_cover_madori_area(facts) or core.brand_name(_pl_brand_id())
...
def _pl_cover_v79_fields(facts, feature_id, layout):
    ...
    #   ★sale-v1：売買は『価格 3,350万円（管理費 …／修繕積立金 …）』を情報バーへ。
    _rentfee = (f"賃料{rent}" + (f"＋管理費{fee}/月" if fee else "")) if rent else ""
    _price_raw = (facts.get("price", "") or "").strip()
    _shuzen = (facts.get("shuzen", "") or "").strip()
    if not rent and _price_raw:
        _sub = "＋".join(x for x in (f"管理費{fee}/月" if fee else "",
                                     f"修繕積立金{_shuzen}/月" if _shuzen else "") if x)
        _rentfee = f"価格{_price_raw}" + (f"（{_sub}）" if _sub else "")
    spec_line = " ｜ ".join(x for x in (f"{madori} {area}".strip(), tag, _rentfee) if x.strip())
    # ★sale-v1：金額は core.money_fields が唯一の情報源（core と app の2箇所で組まない）。
    _mf = core.money_fields(facts)
    price = _mf["price"]
    ...
    kw = {"price": price, "spec_line": spec_line, "equip_line": equip_line, "note_line": note_line,
          ...
          "brand_text": _pl_brand_masthead()}      # ★brand-v1：表紙のマストヘッド誌名
    if layout == "price_hero":
        kw.update({"area_line": area_line, "hook": hook, "price_sub": _mf["price_sub"]})
        ...
        kw.update({"copy1": copy1, "copy2": copy2,
                   "price_sub": (f"{_mf['price_sub']} ／ {area_line}"
                                 if _mf["price_sub"] else area_line)})
    return kw
...
_PL_V_KEEP_KEYS = ("pl_v_model", "pl_v_dur", "pl_v_bgm", "pl_v_aspect", "pl_v_caps",
                   ...
                   "pl_brand_sel",                   # ★brand-v1：誌名の選択も往復で保持
...
def _pl_stage_video():
    ...
    with st.expander(_mag_title, expanded=not _mag_done):
        ...
        with _ic1:
            # ★issue-gate-v1：既定を 1 → 0（未設定）。0 のままだと🎬が押せない（下の gate）。
            st.number_input("ISSUE番号（0＝未設定）", min_value=0, max_value=99, step=1,
                            value=int(_pl_v_keep("pl_issue_no", 0)), key="pl_issue_no",
                            help="毎号ここに入れます。セッションが切れると 0 に戻ります（前回値の自動引き継ぎは"
                                 "別物件の号数を持ち込む事故になるので、あえてしていません）。")
        with _ic2:
            st.text_input("エリア表記（空欄＝マイソクの駅名から自動）",
                          value=_pl_v_keep("pl_issue_area", ""), key="pl_issue_area",
                          placeholder="例: NISHIKUJO")
        # ── ★brand-v1：誌名（マストヘッド1行目）。既定は用途モードに連動・手動で上書きできる。
        _bopts = ["auto"] + list(core.BRAND_ORDER)
        _bauto = core.brand_for_mode(st.session_state.get("pl_mode", ""))
        st.selectbox(
            "誌名（マストヘッド）", _bopts, key="pl_brand_sel",
            index=_bopts.index(_pl_v_keep("pl_brand_sel", "auto"))
            if _pl_v_keep("pl_brand_sel", "auto") in _bopts else 0,
            format_func=lambda b: (f"用途モードに合わせる（今は {core.brand_display_name(_bauto)}）"
                                   if b == "auto" else core.brand_display_name(b)))
        # 手動選択は pl_brand へ（auto のときは空＝_pl_brand_id が用途モードから決める）
        st.session_state["pl_brand"] = ("" if st.session_state.get("pl_brand_sel", "auto") == "auto"
                                        else st.session_state["pl_brand_sel"])
        # ★$3.15を払う前に『実際に焼かれる文字列』を目で確定できる（pregen-guardと同じ思想）。
        if _pl_issue_ready():
            st.caption(f"誌名：**{_pl_brand_masthead()}**　／　マストヘッド表記：**{_pl_issue_text()}**"
                       "（表紙・全ビート・DATA面の3面に同じ文字列が入ります）")
        else:
            st.error("🔴 ISSUE番号が未設定です（0）。入れるまで🎬ルームツアーの生成はできません。"
                     "3面に焼かれ、生成後に直すと fal が再課金になるため、ここで止めています。")
        st.button(("📖 動く雑誌の文字を生成（1コール・特集ベース）" if not _mag_done
                  ...
                  on_click=_pl_mag_generate_cb, args=(_mag_fid, _pl_brand_id()),
            ...
            if not _csrc or not _csrc.get("gen_bytes"):
                ...
                try:
                    with st.spinner("表紙を生成中…（ffmpeg/PIL）"):
                        ...
                        #   ★sale-v1：売買の price / shuzen もチェック対象に入れる
                        #     （賃貸で効いている安全網が売買で無効になっていた）。
                        _mchk = (("賃料", _cfacts.get("rent", "")), ("管理費", _cfacts.get("fee", "")),
                                 ("価格", _cfacts.get("price", "")),
                                 ("修繕積立金", _cfacts.get("shuzen", "")))
                        _bad = [lbl for lbl, v in _mchk if v and not core.money_is_clean(v)]
                        if _bad:
                            _vals = " / ".join(f"{lbl}『{v}』" for lbl, v in _mchk if v)
                            raise ValueError(
                                f"{' / '.join(_bad)}に数字以外の文字が含まれます（抽出値 {_vals}）。"
    ...
    _glob = {"out_w": _ow, "out_h": _oh, "duration": v_dur, "model_key": v_model,
             ...
             # ★brand-v1：誌名。ISSUE番号と同じく job_id(glob) に入る＝ブランドを変えたら別ジョブ
             #   （表紙・全ビート・DATA面の描画が全部変わるのでキャッシュを流用してはいけない）。
             "v79_brand": _pl_brand_masthead(),
             ...
             "data_page": {"feature_id": st.session_state.get("pl_feature", "normal"),
                           ...
                           "brand_text": _pl_brand_masthead(),   # ★brand-v1：DATA面の誌名
    ...
    if not _resumable:   # 新規生成（再開は上の独立ボタン。全成功/未生成はこちら）
        ...
        # ★issue-gate-v1：ISSUE番号が未設定でも封鎖（ui-stepgate-v1 と同じ型）。
        #   セッション切れで既定に戻ったことに気づかず生成→3面に誤号数→直すと fal 再課金、を止める。
        _issue_ok = _pl_issue_ready()
        if gcol.button("🎬 ルームツアーを生成", type="primary", key="pl_v_gen",
                       use_container_width=True, disabled=not (_mag_ready and _issue_ok)):
            _run_video_job()
        if not _mag_ready:
            gcol.caption("⚠️ 先に 📖『動く雑誌の文字を生成』を実行してください"
                         "（未実行だと masthead・部屋タグ・金色見出しが付かない動画になります）。")
        if not _issue_ok:
            gcol.caption("🔴 ISSUE番号が未設定です。📖 の『ISSUE番号』に今回の号数を入れてください"
                         "（表紙・全ビート・DATA面の3面に焼かれ、生成後に直すと fal が再課金になります）。")
