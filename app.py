...
def _pl_pregen_summary(adopted):
    ...
    # ★feat-merge-1：label は表紙の枠に焼く文字（normalは空）なので、人が読む欄は feature_label を使う。
    _parts.append(f"特集={core.feature_label(_fid)}")
...
def _pl_stage_video():
    ...
    with st.expander(_mag_title, expanded=not _mag_done):
        ...
        st.caption(f"特集『{core.feature_label(_mag_fid)}』の文字面を生成"
    ...
    with st.expander("🖼️ 表紙特大（リールカバー / カルーセル1枚目）を生成", expanded=False):
        ...
        # ★v79-3 特集セレクタ（表紙PNG・DATA面・ビートoverlayのaccent／magtextのトーンが参照）。既定 mote_heya。
        #   ★feat-merge-2 までは 内観stagingは pl_concept 側が情報源＝ここを変えても画像プロンプトは変わらない
        #     （app.py:1297・v79-3当時のコメント『staging が参照』は実装と一致していなかったので削除した）。
        #   ★feat-merge-1：並びは FEATURE_ORDER（dictのキー順に依存しない）。表示名は feature_label
        #     （FEATURES[k]["label"] は表紙の枠に焼く文字で normal は空＝そのまま出すと空欄の選択肢になる）。
        st.session_state.setdefault("pl_feature", "mote_heya")
        _fid = st.selectbox("特集（テイスト）", core.FEATURE_ORDER, key="pl_feature",
                            format_func=core.feature_label)
    ...
    _jst_ym = _dtm.now(_tz(_td(hours=9)))
    _data_ym = core.data_note_date(_data_facts, f"{_jst_ym.year}年{_jst_ym.month}月")   # ★注記年月＝マイソク日付優先→生成日
    _data_notes = ["※保証会社利用必須ほか諸費用あり。詳細はお問い合わせください。",
                   "※家具・小物はAI生成のイメージ　※間取り図は資料に基づく",
    ...
    _glob = {"out_w": _ow, "out_h": _oh, "duration": v_dur, "model_key": v_model,
             ...
             "data_page": {"feature_id": st.session_state.get("pl_feature", "mote_heya"),
